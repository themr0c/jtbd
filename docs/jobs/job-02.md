# Job 02 — Plan deployment resources

## Metadata

| Field | Value |
| --- | --- |
| Category | Plan |
| Priority | 🟡 Normal |
| Jira |  |
| Sheet row | Job #2 |
| Status | Not started |

## Job statement

When preparing for installation, I want to identify the necessary compute and database resources so that I can ensure the platform performs reliably at scale.

## Current state

No source references recorded.

## Target structure

| Source section | Proposed JTBD heading |
| --- | --- |
| About Red Hat Developer Hub (Ch 4) | Sizing and system requirements for stable and performant deployments |

## Rewrite instructions

Each heading must express an action + outcome (why the reader does this step).
Replace noun-phrase titles with outcome-oriented equivalents.
Keep all technical steps intact — only headings change.

Apply to: _Confirm source file with CS_

## Acceptance criteria

- [ ] All headings follow action + outcome pattern
- [ ] No heading is a noun phrase only ("Installation", "Prerequisites")
- [ ] Source sections from the table above are all addressed
- [ ] PR linked to this Jira task
- [ ] Brief: https://github.com/themr0c/jtbd/blob/main/docs/jobs/job-02.md
